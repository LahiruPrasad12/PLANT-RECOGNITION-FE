<template>
  <ion-modal ref="modal" :is-open="is_model_open" trigger="open-modal">
    <ion-content>
      <ion-toolbar>
      ...
      <v-form v-slot="{ values, errors }" @submit="saveData">
        <ion-list>
          <ion-item>
            ...
            <ion-input :disabled="true" :value="form.id" autofocus clear-input spellcheck type="text">
            </ion-input>
          </ion-item>
          <ion-item>
            <ion-label position="floating">*Item</ion-label>
            <v-field v-slot="{ field }" :rules="isRequired" name="item">
              <ion-select :compareWith="compareWith" :interface-options="customAlertOptions" interface="alert"
                          name="item" placeholder="Select Item" v-bind="field"
          ...
          <v-error-message class="error" name="item"/>
          <ion-item>
            <ion-label position="floating">*Qty</ion-label>
            <v-field v-slot="{ field }" :rules="isRequired" name="qty">
              <ion-input :value="form.qty" maxlength="10" min="0"
                         name="qty" placeholder="e.g. 12" type="number" v-bind="field"
                         @ionInput="form.qty = $event.target.value" @keydown="$event.key === '-' ? $event.preventDefault() : null">
              </ion-input>
            </v-field>
          </ion-item>
          <v-error-message class="error" name="qty"/>
